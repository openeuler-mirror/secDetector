...
static ssize_t secDetector_log_read(struct file *file, char __user *buffer,
				    size_t buflen, loff_t *fpos)
{
	...
	read_len =
		kfifo_out_spinlocked(&g_secDetector_log_fifo->log_fifo, &log, 1,
				     &g_secDetector_log_fifo->log_fifo_lock);
	if (read_len != 1)
		return -1;
...
static ssize_t secDetector_log_write(struct file *filp,
...
static int create_log(size_t log_size, struct proc_dir_entry *parent)
{
	...
	ret = kfifo_alloc(&g_secDetector_log_fifo->log_fifo, log_size,
			  GFP_KERNEL);
	if (ret != 0) {
	...
	g_proc_alarm = proc_create(ALARM_FILE, ALARM_FILE_MASK, parent,
				   &g_proc_log_file_operations);
	if (!g_proc_alarm) {
